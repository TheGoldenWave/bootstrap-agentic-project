...
- 📌 **双层迭代看板**：新增 `docs/迭代总览.kanban` 项目级总览板 + `docs/prd/{feature_id}/.artifacts/iteration.kanban` feature 级执行板，和 `/progress` 工作流配套使用。
...
│   │   │   ├── process_template.md   # 进度主文档模板（含甘特图）
│   │   │   └── iteration.kanban      # feature 级执行看板模板
...
│   │   └── .artifacts/iteration.kanban # 📌 feature 级执行板
│   └── design/tokens/base.json  # 🎨 Design Token 基准
│
├── tests/specs/                 # 🕵️ 验收测试
└── src/                         # 👨‍💻 业务代码
...
初始化后：
- 项目级总览板：`docs/迭代总览.kanban`
- feature 级执行板：`docs/prd/1.0.0-用户登录-202604/.artifacts/iteration.kanban`
- 维护原则：项目级看板只维护 feature 汇总状态，详细任务在 feature 级执行板维护
