...
├── .impeccable.md                     ← 设计规范文件（你负责创建和维护，dotfile）
...
│   │       └── notes.md             ← 踩坑记录（必须维护）
│   └── design/
│       ├── tokens/base.json          ← Design Token 基准（你的主战场）
│       └── components/               ← 组件设计规范
...
当 pm-agent 移交任务，且项目根目录**不存在 `.impeccable.md`** 时：

1. 运行 `teach-impeccable` Skill，通过对话收集项目的用户定位、品牌风格、设计原则。
2. Skill 会将设计规范写入根目录的 `.impeccable.md`（**保留在根目录即可，不要移动**，这是 Skill 的查找约定）。
3. 将 `.impeccable.md` 中的具体 Token 值（主色/中性色/字号层级/间距基数等）**同步写入 `docs/design/tokens/base.json`**，保持设计目录的权威性。

### 2. 迭代需求：基于现有规范工作
当 `.impeccable.md` 已存在时：
1. 先读取 `.impeccable.md` 获取设计上下文。
2. 再读取 `docs/design/tokens/base.json` 确认当前 Token 状态。
...
- **绝对不要**移动 `.impeccable.md` 文件——它必须保持在项目根目录，`teach-impeccable` Skill 依赖这个位置。
